export function extractDescription(summaryResponse: string): string | null {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('##') && line.toLowerCase().includes('introduction')) {
            // Check for title in same ## line
            const sameLineMatch = line.match(/##.*introduction:?[\s]*([^\n#]*)/i);
    ...
    for (const line of lines) {
        ...
        const afterBoldMatch = trimmed.match(/^\*\*.*?introduction:\s*\*\*\s*(.+)/i);
        if (afterBoldMatch?.[1]) return afterBoldMatch[1].trim();

        // Case 2: Title INSIDE bold (**Title: My Title**)
        const insideBoldMatch = trimmed.match(/^\*\*.*?introduction:\s*([^*]+)\*\*/i);
